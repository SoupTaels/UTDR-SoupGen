...
draw = method(self, function() {
	...
	draw_sprite_ensure(face, index, round(x + xoff), round(y), 1, 1, 0, destroying ? c_red : c_white, alpha);
	var xx = x + ( sprite_get_width(face)/ 2 ) + 10, yy = y - ( sprite_get_height(face)/ 2 );
	scrib.blend(destroying ? c_red : c_white, alpha).starting_format(font).draw(round(xx + xoff), round(yy));
});